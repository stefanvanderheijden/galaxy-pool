...
// Drift Flight: thrust is expressed relative to the direction of travel.
//   W = prograde   (thrust along velocity — speed up)
//   S = retrograde (thrust against velocity — brake)
//   A = port       (thrust 90° left of travel — steer the path left)
//   D = starboard  (thrust 90° right of travel — steer the path right)
//   Spacebar       = full retrograde brake (clamped so it can't reverse)
// When nearly stationary there is no travel direction, so we fall back to the
// ship's current facing (shipAngle) as the reference frame.
...
function applyShipInput(dt_yr, realDt_s) {
  ...
  // Spacebar = full retrograde brake, clamped so it can't push past a standstill.
  if (keys.space) {
    if (speed > 1e-6) {
